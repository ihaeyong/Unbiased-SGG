...
from .loss import ClassBalancedLoss, FocalLoss, ReweightLoss
...
class BGNNPredictor(nn.Module):
    ...
    def forward(
        ...
        if self.training and self.use_nbdt_loss:
            ...
            labels = cat(rel_labels, dim=0)
            assert rel_dists.shape[0] == labels.shape[0]
            nbdt_loss = self.nbdt_loss(rel_dists, labels, self.config.MODEL.ROI_RELATION_HEAD.LOSS.NBDT.MODE)
        ...
        return obj_pred_logits, rel_cls_logits, add_losses, freq_bias
...
class TransformerPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        # module construct
        self.obj_context_layer = Transformer_ObjContext(config, obj_classes, in_channels)
    ...
    def feature_generate(self, roi_features, proposals, num_objs, rel_pair_idxs, union_features, num_rels, logger):
        ...
        return rel_feats, pair_pred, obj_dists, pair_obj_embed
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        rel_feats, pair_pred, obj_dists, prod_emb = self.feature_generate(roi_features, proposals, num_objs, rel_pair_idxs, union_features, num_rels, logger)
...
class VCTreePredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        # encode context infomation
        obj_dists, obj_preds, edge_ctx, binary_preds, obj_embed = self.context_layer(roi_features, proposals, rel_pair_idxs, logger)
        ...
        if True:
            ...
            rel_dists = ctx_dists + freq_dists
