...
class UnionRegionAttention(nn.Module):
    ...
    def __init__(self, obj_dim=256, rib_scale = 1, power=1, cfg=None):
        ...
        subjobj_upconv = [
            ...
            #nn.BatchNorm2d(16),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(32, 8, 3, bias=False),
            #nn.BatchNorm2d(8),
            nn.ReLU(inplace=True),
        ]
        ...
        if self.rib_scale == 1:
            ...
            subjobj_mask = [
                nn.Conv2d(8*3, 3, 1, stride=1, bias=False),
        ...
        if self.rib_scale == 2:
            subjobj_mask = [
                nn.ConvTranspose2d(8*3, 3, 3, stride=2, padding=1, bias=False),
                #nn.BatchNorm2d(16),
                nn.Conv2d(3,1,1,stride=1, bias=False),
                nn.Sigmoid(),]
            self.subjobj_mask = nn.Sequential(*subjobj_mask)
        ...
        if self.g_type is 'conv':
            ...
            self.iba = PerSampleBottleneck(sigma=1.0 if self.fmap_size == 25 else 0.5,
